...
class AccountDialog:
    ...
    def setup_ui(self):
        ...
        # Add Token Management Tab after Basic Info
        token_frame = ttk.Frame(notebook, padding="10")
        notebook.add(token_frame, text="Token Management")
        
        # Real Token
        ttk.Label(token_frame, text="Real Token:").grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        self.real_token_entry = ttk.Entry(token_frame, width=50, show="*")
        self.real_token_entry.grid(row=0, column=1, columnspan=2, sticky=tk.EW, pady=(0, 10))
        
        # Show/Hide real token button
        self.show_real_token_btn = ttk.Button(token_frame, text="Show", command=self.toggle_real_token_visibility)
        self.show_real_token_btn.grid(row=0, column=3, padx=(5, 0), pady=(0, 10))
        
        # Token status
        self.token_status = ttk.Label(token_frame, text="", foreground='gray')
        self.token_status.grid(row=1, column=1, columnspan=2, sticky=tk.W, pady=(0, 10))
        
        # Token actions
        ttk.Button(token_frame, text="Test Token", command=self.test_real_token).grid(row=2, column=1, sticky=tk.W, pady=(0, 10))
        ttk.Button(token_frame, text="Clear Token", command=self.clear_real_token).grid(row=2, column=2, sticky=tk.W, pady=(0, 10))
        
        # Help text
        help_text = ("• Real token is obtained automatically after sign-in\n"
                    "• You can manually edit or clear the token\n"
                    "• Clearing token will trigger new sign-in on next run\n"
                    "• Test token to verify if it's still valid")
        ttk.Label(token_frame, text=help_text, justify=tk.LEFT, font=('TkDefaultFont', 8)).grid(
            row=3, column=1, columnspan=2, sticky=tk.W, pady=(10, 0))
    ...
    def toggle_real_token_visibility(self):
        if self.real_token_entry.cget("show") == "*":
            self.real_token_entry.config(show="")
            self.show_real_token_btn.config(text="Hide")
        else:
            self.real_token_entry.config(show="*")
            self.show_real_token_btn.config(text="Show")
    
    def test_real_token(self):
        real_token = self.real_token_entry.get().strip()
        if not real_token:
            messagebox.showwarning("Warning", "No real token to test!")
            return
        ...
        try:
            ...
            response = requests.get("http://tuberocket.app:3000/api/member", headers=headers, timeout=10)
            data = response.json()
            
            if response.status_code == 200 and data.get("retMessage") != "Token Invalid":
                email = data["result"]["email"]
                coin = data["result"]["coin"]
                self.token_status.config(text=f"✅ Token valid - {email} | {coin} coins", foreground='green')
                messagebox.showinfo("Success", f"Token is valid!\nEmail: {email}\nCoins: {coin}")
            else:
                self.token_status.config(text="❌ Token invalid", foreground='red')
                messagebox.showerror("Error", f"Token is invalid!")
        except Exception as e:
            self.token_status.config(text="❌ Test failed", foreground='red')
            messagebox.showerror("Error", f"Test failed: {str(e)}")
    
    def clear_real_token(self):
        if messagebox.askyesno("Confirm", "Clear real token?\nThis will trigger new sign-in on next run."):
            self.real_token_entry.delete(0, tk.END)
            self.token_status.config(text="Token cleared - Will get new on next run", foreground='orange')
    ...
    def load_account_data(self):
        ...
        if 'token' in self.account_data and self.account_data['token']:
            self.real_token_entry.insert(0, self.account_data['token'])
            self.token_status.config(text="✅ Real token loaded", foreground='green')
        else:
            self.token_status.config(text="No real token - Will get from sign-in", foreground='gray')
    ...
    def save_account(self):
        ...
        self.result = {
            ...
            'token': self.real_token_entry.get().strip(),  # Include real token in result
